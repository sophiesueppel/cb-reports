...
OUTPUT_PATH = Path("report_ecb.html")

ECB_REPORT_URL = "https://claude.ai/code/artifact/6fe098d3-f129-4f6b-8891-0860dea9251b"
...
<title>ECB Policy Sentiment Tracker</title>
<style>
...
.eyebrow{{font-size:10px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:#003087;margin-bottom:5px}}
...
<header>
  <div>
    <div class="eyebrow">European Central Bank</div>
...
document.getElementById('tbody').innerHTML=byDate.map(d=>{{
  ...
  const titleCell=d.url&&!d.url.startsWith('ecb::')
    ?`<a href="${{d.url}}" target="_blank" onclick="event.stopPropagation()">${{d.title}}</a>`
    :d.title;
  return`<tr>
    <td class="td-date">${{fmtShort(d.date)}}</td>
    <td class="td-speaker" title="${{d.speaker}}">${{d.speaker}}</td>
    <td>
      <div class="title-text">${{titleCell}}</div>
...
def generate_ecb_report() -> None:
    ...
    df = pd.read_sql(
        ...
        "WHERE central_bank='ECB' AND score IS NOT NULL AND date >= ? "
        "ORDER BY date DESC",
        conn,
        params=(cutoff,),
    )
    ...
    # Filter to speeches where the speaker was actually on the Executive Board on that date
    from membership import was_member
    df = df[df.apply(lambda r: was_member("ecb", r["speaker"], r["date"]), axis=1)].copy()

    if df.empty:
        print("No rated ECB Executive Board speeches yet — skipping ECB report generation.")
        return
    ...
    active_members_json = json.dumps(sorted(EXEC_BOARD))
    ...
    print(f"ECB report written to {OUTPUT_PATH.resolve()}")
