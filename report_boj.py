...
OUTPUT_PATH = Path("report_boj.html")

BOJ_REPORT_URL = "https://claude.ai/code/artifact/770501ac-36f2-455c-a465-187c89d67004"
...
<title>BoJ Policy Sentiment Tracker</title>
<style>
...
.eyebrow{{font-size:10px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:#BC002D;margin-bottom:5px}}
...
<header>
  <div>
    <div class="eyebrow">Bank of Japan</div>
...
<section>
  <div class="section-header">
    <span class="section-title">Policy Board Speeches &middot; Last 5 Years &middot; Click row to expand</span>
    <div class="section-rule"></div>
...
if(latest){{
  ...
  <div class="latest-card">
    <div class="card-label">Most Recent Policy Board Speech</div>
    <div>
...
def generate_boj_report() -> None:
    ...
    df = pd.read_sql(
        ...
        "WHERE central_bank='Bank of Japan' AND score IS NOT NULL AND date >= ? "
        "ORDER BY date DESC",
        conn,
        params=(cutoff,),
    )
    ...
    if df.empty:
        print("No rated BoJ Policy Board speeches in the last 5 years — skipping BoJ report generation.")
        return
    ...
    from meetings import BOJ_MEETINGS
    timeline_html = make_timeline(df, meetings=BOJ_MEETINGS)
    trend_html = make_trend_chart(df)

    from scraper_boj import BOJ_POLICY_BOARD
    active_members_json = json.dumps(sorted(BOJ_POLICY_BOARD))
    ...
    print(f"BoJ report written to {OUTPUT_PATH.resolve()}")
